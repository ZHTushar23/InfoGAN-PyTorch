...
'''
'''
11 BIT Discrete code. 4 sza+ 7 vza= 11
'''
...
class NasaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='infoGAN_cot':
            ...
            # convert to tensor
            dis_c_tensor =  self.transform1(dis_c)
        ...
        if self.mode in ["cot","infoGAN_cot","infoGAN_cot_dc"]:
        ...
        if self.mode in ["infoGAN","infoGAN_cot","infoGAN_cot_dc"]:
            input_img = torch.cat((input_img, dis_c_tensor), dim=0)
...
if __name__=="__main__":
    ...
    for fold in range(1):    
        ...
        train_data = NasaDataset(root_dir=root_data_dir,filelist=data_split_dir+"data_split/cot_all/train.csv",mode="infoGAN_cot_dc")
        ...
        for i, data in enumerate(loader):
            ...
            break
    # data_mean, data_std = torch.tensor([0.1740, 0.1207], dtype=torch.float64), torch.tensor([0.2326, 0.1338], dtype=torch.float64)
    # transform_func = T.Compose([T.Normalize(mean=data_mean, std=data_std)])
        
    # train_data = NasaDataset(root_dir=root_data_dir,
    # filelist=data_split_dir+"data_split/cot_all/train.csv",
