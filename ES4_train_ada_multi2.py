...
from ES4_config import params
from ES4_dataset_multi import NasaDataset
...
saved_model_root_dir = "ES4_saved_model_multi2"
try:
...
data_split_sub_dir = "data_split/mcot_sza_20_vza_all/"
...
del state_dict
for fold in range(2):
    saved_model_dir = saved_model_root_dir+"/fold_%01d_multi2"%(fold)
    try:
    ...
    for epoch in range(params['num_epochs']):
        ...
        for i, data_batch in enumerate(loader, 0):
            ...
            # get translated radiance using the p_model/ 1st stage model
            noise = noise.to(dtype=torch.float32)
            style_code = style_code.to(dtype=torch.float32)
            
            with torch.no_grad():
                stage1_output = p_model(noise,style_code)


            stage1_output = stage1_output.to(device,dtype=torch.float32)
            style_code = style_code.to(device,dtype=torch.float32)

            fake_data = netG(stage1_output,style_code)            
        ...
        for i, data_batch in enumerate(val_loader):
            ...
            with torch.no_grad():
                generated_img1 = netG(stage1_output,style_code).detach().cpu()            
