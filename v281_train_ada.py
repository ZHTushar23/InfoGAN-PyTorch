...
from v28_dataset import NasaDataset
...
saved_model_root_dir = "v281_saved_model/sza_"+str(params['sza_list2'])+"_vza_"+str(params['vza_list2'])
try:
...
data_mean, data_std = torch.tensor([0.1330, 0.1087], dtype=torch.float64), torch.tensor([0.1537, 0.0924], dtype=torch.float64)
...
for fold in range(2):
    ...
    for epoch in range(params['num_epochs']):
        ...
        for i, data_batch in enumerate(loader, 0):
            ...
            gen_loss = criterionD(probs_fake, label)+100*criterionR(fake_data, real_data)
