...
elif(params['dataset'] == 'Cloud4'):
    # Need to decide new models
    # Consider the input, output size of each blocks
    from models.cloud_model4 import Generator, Discriminator, DHead, QHead
...
cf1 = sample_batch['sza_emb'][:2]
cf1 = cf1.to(device,dtype=torch.float32)
cf2 = sample_batch['vza_emb'][:2]
...
for epoch in range(params['num_epochs']):
    ...
    for i, data_batch in enumerate(dataloader, 0):
        ...
        c1, c2     = data_batch['sza_emb'], data_batch['vza_emb']
        noise = noise.to(device,dtype=torch.float32)

        c1 = torch.unsqueeze(c1,0)
        c2 = torch.unsqueeze(c2,0)

        c1 = c1.to(device,dtype=torch.float32)
        c2 = c2.to(device,dtype=torch.float32)

        fake_data = netG(noise, c1, c2)
    ...
    with torch.no_grad():
        gen_data = netG(fixed_noise, cf1, cf2).detach().cpu()
